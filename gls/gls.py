...
def create_testbench(app, inputs, outputs, input_widths, output_widths, num_test_vectors, active_cycle_number, delayed=0):
    ...
    for o in outputs:
        output_checks += f'''
        if (({o} != test_outputs[test_vector_addr][`SLICE_{o.upper()}] && {o} != test_outputs[test_vector_addr + `DELAYED][`SLICE_{o.upper()}]) || ($isunknown({o}) && test_vector_addr != 0)) begin
    ...
    tb_file = 'testbench' + ('_delay' if delayed else '') + '.sv'
    open(tb_file, 'w').write(output)
