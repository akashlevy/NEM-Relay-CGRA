...
####done_fp
####add_core_fiducials
####create_route_blockage -layers all -rects {{1840 2930} {2740 2970}}       
####create_route_blockage -layers all -rects {{1840 2730} {2740 2770}}       
####create_place_blockage -rects {{1840 2930} {2740 2970}}       
####create_place_blockage -rects {{1840 2730} {2740 2770}} 
####
####source ../../scripts/vlsi/flow/scripts/gen_floorplan.tcl
####set_multi_cpu_usage -local_cpu 8
####gen_bumps
...
####       create_shape -net VDD -layer M8 -rect [list 80 $a  4835 $a1] -shape stripe -status fixed
####       create_shape -net VSS -layer M8 -rect [list 80 $a2 4835 $a3] -shape stripe -status fixed
####      }
####    } else {
####      for {set z 0} {$z < 16} {incr z} {
####       set a  [expr $y_loc + ($z*12) + 4 + 0]
...
####  set l0 [expr [lindex $bbox1 0] + 0.05] 
####  set l1 [expr [lindex $bbox1 1] + 0.05] 
####  set l2 [expr [lindex $bbox1 2] - 0.05] 
####  set l3 [expr [lindex $bbox1 3] - 0.05] 
...
####
####eval_legacy {
####editCutWire -box [list 1840 2930 2740 2970] -only_visible_wires
####gui_select -rect [list 1840 2930 2740 2970]
####deleteSelectedFromFPlan
####}
####eval_legacy {
####editCutWire -box [list 1840 2730 2740 2770] -only_visible_wires
####gui_select -rect [list 1840 2730 2740 2770]
####deleteSelectedFromFPlan
####}
####
####write_db init3.db
...
for {set y_loc 110} {$y_loc < 580} {incr y_loc 20} {
  create_shape -net VDD -layer M8 -rect [list 80 $y_loc 4835 [expr $y_loc + 3]] -shape stripe -status fixed
  create_shape -net VSS -layer M8 -rect [list 80 [expr $y_loc + 4] 4835 [expr $y_loc + 7]] -shape stripe -status fixed
}
...
for {set x_loc 110} {$x_loc < 580} {incr x_loc 20} {
  create_shape -net VDD -layer M7 -rect [list $x_loc 100 [expr $x_loc + 1] 4835] -shape stripe -status fixed
  create_shape -net VSS -layer M7 -rect [list [expr $x_loc + 1.5] 100 [expr $x_loc + 2.5] 4835] -shape stripe -status fixed
}
for {set x_loc 4450} {$x_loc < 4780} {incr x_loc 20} {
  create_shape -net VDD -layer M7 -rect [list $x_loc 100 [expr $x_loc + 1] 4835] -shape stripe -status fixed
  create_shape -net VSS -layer M7 -rect [list [expr $x_loc + 1.5] 100 [expr $x_loc + 2.5] 4835] -shape stripe -status fixed
}
